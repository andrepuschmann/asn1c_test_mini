...
OCTET_STRING_decode_aper(asn_codec_ctx_t *opt_codec_ctx,
	...
	ASN_DEBUG("APER Decoding %s size %lld .. %lld bits %d",
		csiz->flags & APC_EXTENSIBLE ? "extensible" : "non-extensible",
		csiz->lower_bound, csiz->upper_bound, csiz->effective_bits);
	...
	do {
		...
		/* Get the PER length */
		raw_len = aper_get_length(pd, csiz->effective_bits, &repeat);
		if(raw_len < 0) {
			RETURN(RC_WMORE);
		}
		raw_len += csiz->lower_bound;
...
OCTET_STRING_encode_aper(asn_TYPE_descriptor_t *td,
	...
	if(csiz->effective_bits >= 0) {
		...
		if (st->size > 2) {
			/* insert align bits if size are greater than 2 octet */
			ret = aper_insert_align_bits(po);
			if (ret) {
				ASN__ENCODE_FAILED;
			}
		}
	...
	buf = st->buf;
	while(sizeinunits) {
		ssize_t maySave = aper_put_length(po, sizeinunits);
		if(maySave < 0) ASN__ENCODE_FAILED;
